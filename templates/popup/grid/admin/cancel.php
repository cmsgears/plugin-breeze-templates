...
$url		= $data[ 'url' ];
?>
<div id="popup-grid-cancel" class="popup popup-admin popup-medium <?= $modal ? 'popup-modal' : null ?>">
	...
	<div class="popup-data <?= isset( $widget->size ) ? "popup-data-$widget->size" : null ?>">
		...
		<div class="popup-content">
			...
			<form cmt-app="<?= $app ?>" cmt-controller="<?= $controller ?>" cmt-action="<?= $action ?>" action="<?= $url ?>">
				...
				<div class="clear filler-height"></div>
				<div class="align align-center"><input class="element-medium" type="submit" value="Cancel" /></div>
			</form>
		</div>
	</div>
</div>