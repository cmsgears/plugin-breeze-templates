<?php
/**
 * This file is part of CMSGears Framework. Please view License file distributed
 * with the source code for license details.
 *
 * @link https://www.cmsgears.org/
 * @copyright Copyright (c) 2015 VulpineCode Technologies Pvt. Ltd.
 */

namespace cmsgears\templates\breeze\models\widget\newsletter;

// Yii Imports
use Yii;

/**
 * NewsletterConfig provide widget config data of Newsletter Widget.
 *
 * @since 1.0.0
 */
class NewsletterConfig extends \cmsgears\templates\breeze\models\widget\WidgetConfig {

	// Variables ---------------------------------------------------

	// Globals -------------------------------

	// Constants --------------

	// Public -----------------

	// Protected --------------

	// Variables -----------------------------

	// Public -----------------

	public $split		= false;
	public $splitClass	= 'widget-newsletter-split';	
	
    public $btnText	= "Join Us";

	public $labels = false;

	public $spinner = 'cmti cmti-3x cmti-spinner-10';

	public $ajaxUrl	= 'newsletter/site/sign-up'; // CMT App Request - Submit Path

	// CMT JS Framework to handle ajax request
	public $cmtApp			= 'core';
    public $cmtController	= 'default';
    public $cmtAction		= 'default';

	// Protected --------------

	// Private ----------------

	// Traits ------------------------------------------------------

	// Constructor and Initialisation ------------------------------

	// Instance methods --------------------------------------------

	// Yii interfaces ------------------------

	// Yii parent classes --------------------

	// yii\base\Component -----

	// yii\base\Model ---------

	/**
	 * @inheritdoc
	 */
	public function rules() {

		$rules = parent::rules();

		$rules[] = [ [ 'split', 'labels' ], 'boolean' ];		
		$rules[] = [ [ 'splitClass', 'btnText', 'spinner', 'cmtApp', 'cmtController', 'cmtAction' ], 'string', 'min' => 1, 'max' => Yii::$app->core->mediumText ];
		$rules[] = [ [ 'ajaxUrl' ], 'string', 'min' => 1, 'max' => Yii::$app->core->xxLargeText ];

		return $rules;
	}

	/**
	 * @inheritdoc
	 */
	public function attributeLabels() {

		return [
			'btnText' => 'Button Text',
			'spinner' => 'Spinner',
			'ajaxUrl' => 'AJAX Url',
			'cmtApp' => 'AJAX Application',
			'cmtController' => 'AJAX Controller',
			'cmtAction' => 'AJAX Action',
		];
	}

	// CMG interfaces ------------------------

	// CMG parent classes --------------------

	// Validators ----------------------------

	// PagingConfig --------------------------

	public function generateConfig( $params = [] ) {

		$config = parent::generateConfig();

		$config[ 'btnText' ]	= $this->btnText;
		$config[ 'labels' ]		= $this->labels;
		$config[ 'spinner' ]	= $this->spinner;
		$config[ 'ajaxUrl' ]	= $this->ajaxUrl;
		$config[ 'cmtApp' ]		= $this->cmtApp;

		$config[ 'cmtController' ] = $this->cmtController;

		$config[ 'cmtAction' ]	= $this->cmtAction;

		return $config;
	}

}
